...
class ImgWelcome(M.Cog):
	...
	async def _create_welcome(O,member,test_member_number:I=E):
		...
		A2=B(Q)+O._get_suffix(Q);A3=B(K.guild.name)+'!'if D(B(K.guild.name))<=28 else B(K.guild.name)[:23]+'...';N((152,96),f"Ти си {B(A2)} потребител",1,U,M);H.text((152,96),f"Ти си {B(A2)} потребител",font=U,fill=x);N((152,116),AR+A3,1,U,M);H.text((152,116),AR+A3,font=U,fill=x);l=m();L.save(l,format='PNG');l.seek(0);return l
